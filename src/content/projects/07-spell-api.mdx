...
To hone my back end skills, I decided to build a REST API to fetch data from my homebrew D&D 5e spell list. NestJS seemed like a great fit for this use case after my previous experience with it and the reliability it provides.

This project is later going to be consumed by a frontend website for my players to search. For early testing purposes, the database was a SQLite file with which I interacted using TypeORM. I find the concept of SQLite incredible: your database is reduces to its most simple form and doesn't even require a server! Later on, I switched to a MongoDB database hosted on Atlas because I wanted a more flexible schema and the ability to store more complex data in a single field. I used mongoose to interact with the database and create a strict document schema.
...
Search a spell by name in the search bar below.

<Search client:load url="https://nestjs-spells-api.fly.dev/spells" />
